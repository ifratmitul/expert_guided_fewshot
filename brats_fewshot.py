...
class Config:
    ...
    # Few-shot parameters
    N_SHOT = 5   # 3-shot learning
    N_QUERY = 5  # 3 query samples per class
    EMBEDDING_SIZE = 512
...
# --- Enhanced Evaluation function with AUC metrics ---
def evaluate_model(model, prototypes, data_loader, dataset_name="Test"):
    """Evaluate model on dataset with AUC metrics"""
    model.eval()
    
    all_preds = []
    all_labels = []
    all_probabilities = []  # NEW: For AUC calculation
    ...
    with torch.no_grad():
        for images, labels, _ in data_loader:
            ...
            # Convert distances to probabilities for AUC calculation
            probabilities = F.softmax(-dists, dim=1)  # Negative distances to get similarity
    ...
    # NEW: Calculate AUC metrics
    print(f"\n{dataset_name} AUC Scores:")
    print("-" * 40)
    ...
    for class_idx in range(Config.NUM_CLASSES):
        try:
            # Binary classification for each class (one-vs-all)
            ...
            per_class_auc[Config.CLASSES[class_idx]] = class_auc
            auc_scores.append(class_auc)
            
            print(f"  {Config.CLASSES[class_idx]:<20}: {class_auc:.4f}")
            
        except ValueError as e:
            print(f"  {Config.CLASSES[class_idx]:<20}: Error - {e}")
            per_class_auc[Config.CLASSES[class_idx]] = 0.0
            auc_scores.append(0.0)
    
    # Macro AUC (unweighted average)
    if auc_scores:
        macro_auc = np.mean(auc_scores)
        print(f"\n  {'Macro AUC':<20}: {macro_auc:.4f}")
    else:
        macro_auc = 0.0
    
    # Multi-class AUC using sklearn (macro and weighted)
    try:
        sklearn_macro_auc = roc_auc_score(all_labels, all_probabilities, multi_class='ovr', average='macro')
        sklearn_weighted_auc = roc_auc_score(all_labels, all_probabilities, multi_class='ovr', average='weighted')
        print(f"  {'Weighted AUC':<20}: {sklearn_weighted_auc:.4f}")
    except ValueError as e:
        print(f"  Multi-class AUC calculation failed: {e}")
        sklearn_macro_auc = macro_auc
        sklearn_weighted_auc = 0.0
    
    print("-" * 40)
    
    # Save report with AUC metrics
    ...
    with open(report_path, 'w') as f:
        ...
        for class_name, auc_score in per_class_auc.items():
            f.write(f"{class_name:<20}: {auc_score:.4f}\n")
        f.write(f"\nMacro AUC: {macro_auc:.4f}\n")
        f.write(f"Weighted AUC: {sklearn_weighted_auc:.4f}\n")
...
def run_brats_pipeline():
    ...
    n_episodes = Config.NUM_EPISODES  # episodes per epoch
